...
- editor command는 제한된 `CommandContext`와 typed `ChangeSet`을 사용하며 중앙 dispatcher가 cache/dirty/autosave/repaint를 처리한다. 표준 schematic command, page/annotation, compound ERC repair, component/wire drag는 영향받은 entity delta를 저장하고, undo/redo는 16 MiB 제한을 적용한다. full snapshot history는 project/circuit load·recovery·migration 같은 전체 문서 교체 경계에만 남아 있다.
