...
#[derive(Debug, Clone)]
struct BreadboardGuide {
    title: String,
    controller: Option<String>,
    peripheral: Option<String>,
    routes: Vec<BreadboardRoute>,
    notes: Vec<String>,
}

fn build_breadboard_guide(components: &[Component], netlist: &CircuitNetlist) -> BreadboardGuide {
    ...
    let route_specs: Vec<(&str, &str, &'static str)> = match (controller.kind, peripheral.kind) {
        ...
        _ => {
            guide.notes.push(format!(
                ...
                component_kind_label(controller.kind),
                component_kind_label(peripheral.kind)
            ));
            Vec::new()
        }
    };
...
fn render_breadboard_view(ui: &mut egui::Ui, guide: &BreadboardGuide) -> Option<BreadboardRoute> {
    let mut selected_route = None;
    ui.horizontal(|ui| {
        status_pill(ui, "Schematic synced", StatusTone::Live);
        ui.label(
